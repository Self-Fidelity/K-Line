...
- **数据端**：支持 [AkShare](https://www.akshare.xyz/) 与 [Tushare Pro](https://tushare.pro/) 双数据源切换，自动/手动获取 A 股历史行情（日 K、分钟 K），支持增量更新与批量更新。切换数据源后旧数据保留，通过 `data_source` 字段物理隔离。**支持前复权/后复权/不复权数据共存**，通过 `adjust` 字段物理隔离。
...
| 前端 | Vue 3 • TypeScript • Vite 5 • Element Plus • Pinia • Lightweight Charts v5 • CodeMirror 6 • pinyin-pro |
| 数据 | Pandas • NumPy • AkShare • Tushare Pro • Bokeh（图表生成）|
...
│   ├── data_fetcher/          # StockDataFetcher, StockListManager, Provider 抽象层
│   │   └── providers/         # BaseDataProvider / AkShareProvider / TushareProvider
...
**数据源配置**：
- `data_source`：当前激活的数据源，`akshare`（默认）或 `tushare`。存储在数据库 `data_update_config` 表中，可通过前端"数据更新管理"页面切换。
- `tushare_token`：Tushare Pro API Token，同样存储在 `data_update_config` 表中（不入 `.env`/Git）。
- `AKSHARE_CONCURRENCY`：AkShare 并发线程数（默认 4，N100 建议 3-5）。
- `AKSHARE_REQUEST_INTERVAL`：AkShare 单线程请求间隔秒数（默认 0.8，反爬保护）。
...
- `src/data_fetcher/`：
  - `providers/`：**Provider 抽象层**。`BaseDataProvider` 定义统一接口；`AkShareProvider` 封装 akshare（`stock_zh_a_hist`）；`TushareProvider` 封装 tushare pro（`pro.daily()` + `adj_factor` 复权）。`StockDataFetcher` 根据 `data_update_config` 中的 `data_source` 配置自动选择 Provider，支持运行时热切换。
  - 所有数据获取调用集中于此，含重试逻辑与批量处理。
- `src/data_storage/`：
  ...
  - **市场数据表 `stock_daily_kline` 已增加 `data_source TEXT NOT NULL DEFAULT 'akshare'` 字段**，复合主键为 `(stock_code, trade_date, data_source)`，支持 AkShare 与 Tushare 数据物理隔离共存。

---

### 数据库表结构参考（SQLite & PostgreSQL）

> 以下 schema 同时适用于 `SQLiteStorage` 与 `PostgresStorage`。两者差异在文末注明。

#### 市场数据表

**`stock_daily_kline`** — 日K线数据（核心市场数据）

| 列名 | 类型 | 约束 | 说明 |
|------|------|------|------|
| `id` | INTEGER / Serial | PK, AutoIncrement | 自增主键 |
| `stock_code` | TEXT / String(10) | NOT NULL | 股票代码（如 `000001`） |
| `trade_date` | TEXT / String(8) | NOT NULL | 交易日期 `YYYYMMDD` |
| `data_source` | TEXT / String(20) | NOT NULL, DEFAULT `'akshare'` | 数据来源：`akshare` 或 `tushare` |
| `adjust` | TEXT / String(10) | NOT NULL, DEFAULT `'qfq'` | 复权方式：`qfq`(前复权)、`hfq`(后复权)、`none`(不复权) |
| `open` | REAL / Float | NOT NULL | 开盘价 |
| `close` | REAL / Float | NOT NULL | 收盘价 |
| `high` | REAL / Float | NOT NULL | 最高价 |
| `low` | REAL / Float | NOT NULL | 最低价 |
| `volume` | REAL / Float | NOT NULL | 成交量 |
| `amount` | REAL / Float | — | 成交额 |
| `pct_chg` | REAL / Float | — | 涨跌幅(%) |
| `change` | REAL / Float | — | 涨跌额 |
| `turnover` | REAL / Float | — | 换手率 |
| `update_time` | TEXT / String(20) | NOT NULL | 记录更新时间 |

- **复合唯一约束**：`(stock_code, trade_date, data_source, adjust)`
- **索引**：`idx_kline_by_stock` `(stock_code, data_source, adjust, trade_date)`，`idx_kline_by_date` `(trade_date, data_source, adjust, stock_code)`

**`stock_list`** — 股票列表

| 列名 | 类型 | 约束 | 说明 |
|------|------|------|------|
| `code` | TEXT / String(10) | PK | 股票代码 |
| `name` | TEXT / String(100) | NOT NULL | 股票名称 |
| `market` | TEXT / String(20) | NOT NULL | 市场分类：`main`/`cyb`/`kcb`/`all` |
| `update_time` | TEXT / String(20) | NOT NULL | 更新时间 |

#### 策略与参数表

**`stock_strategy_params`** — 策略参数（旧版，保留向后兼容）

| 列名 | 类型 | 约束 | 说明 |
|------|------|------|------|
| `id` | INTEGER / Serial | PK, AutoIncrement | — |
| `stock_code` | TEXT / String(10) | NOT NULL | — |
| `strategy_name` | TEXT / String(100) | NOT NULL | — |
| `params` | TEXT / String | NOT NULL | JSON 序列化后的参数字典 |
| `update_time` | TEXT / String(20) | NOT NULL | — |

- **唯一约束**：`(stock_code, strategy_name)`

**`strategy_param_sets`** — 策略参数集（新版，支持多参数集 + PSO 优化结果）
...
| `stock_code` | TEXT / String(10) | NOT NULL | — |
| `strategy_name` | TEXT / String(100) | NOT NULL | — |
| `name` | TEXT / String(200) | NOT NULL | 参数集名称 |
| `description` | TEXT / String | — | 描述 |
| `params` | TEXT / String | NOT NULL | JSON 参数字典 |
| `param_ranges` | TEXT / String | — | JSON 参数范围（PSO 用） |
| `target_metric` | TEXT / String(50) | — | 优化目标指标 |
| `best_score` | REAL / Float | — | 最佳得分 |
| `optimization_method` | TEXT / String(50) | — | 优化方法，如 `pso` |
| `num_particles` | INTEGER / Integer | — | PSO 粒子数 |
| `max_iter` | INTEGER / Integer | — | PSO 迭代次数 |
| `date_range` | TEXT / String(100) | — | 回测日期范围 |
| `created_at` | TEXT / String(20) | NOT NULL | 创建时间 |
| `is_default` | INTEGER / Integer | DEFAULT 0 | 是否为默认参数集 |

- **唯一约束**：`(stock_code, strategy_name, name)`
- **索引**：`idx_param_sets_stock_strategy` `(stock_code, strategy_name)`，`idx_param_sets_created` `(created_at DESC)`

**`aggregation_schemes`** — 策略聚合方案（多策略加权投票）

| 列名 | 类型 | 约束 | 说明 |
|------|------|------|------|
| `id` | INTEGER / Serial | PK, AutoIncrement | — |
| `name` | TEXT / String(200) | NOT NULL | 方案名称 |
| `description` | TEXT / String | — | 描述 |
| `stock_code` | TEXT / String(20) | — | 关联股票代码（可选） |
| `strategies` | TEXT / String | NOT NULL | JSON 策略权重配置 |
| `buy_threshold` | REAL / Float | NOT NULL | 买入阈值 |
| `sell_threshold` | REAL / Float | NOT NULL | 卖出阈值 |
| `required_strategies` | TEXT / String | NOT NULL | JSON 必需策略列表 |
| `created_at` | TEXT / String(20) | NOT NULL | — |
| `updated_at` | TEXT / String(20) | NOT NULL (SQLite) / nullable (PG) | 更新时间 |

- **索引**：`idx_agg_schemes_name` `(name)`

**`custom_strategies`** — 用户自定义策略（Web 编辑器 + RestrictedPython 沙箱）

| 列名 | 类型 | 约束 | 说明 |
|------|------|------|------|
| `id` | INTEGER / Serial | PK, AutoIncrement | — |
| `user_id` | INTEGER / Integer | NOT NULL | 所属用户 |
| `name` | TEXT / String(200) | NOT NULL | 策略名称 |
| `description` | TEXT / String | DEFAULT `''` | 简短描述 |
| `detailed_description` | TEXT / String | DEFAULT `''` | 详细描述 |
| `code` | TEXT / String | NOT NULL | Python 策略代码 |
| `parameter_descriptions` | TEXT / String | DEFAULT `'{}'` | JSON 参数说明 |
| `file_path` | TEXT / String(500) | — | 本地文件路径（可选） |
| `is_public` | INTEGER / Integer | DEFAULT 0 | 是否公开 |
| `is_system` | INTEGER / Integer | DEFAULT 0 | 是否系统内置 |
| `created_at` | TEXT / String(20) | NOT NULL | — |
| `updated_at` | TEXT / String(20) | — | 更新时间 |
...
|------|------|------|------|
| `id` | INTEGER / Serial | PK, AutoIncrement | — |
| `username` | TEXT / String(50) | NOT NULL, UNIQUE | 用户名 |
| `hashed_password` | TEXT / String(255) | NOT NULL | bcrypt 哈希密码 |
| `email` | TEXT / String(255) | — | 邮箱 |
| `role` | TEXT / String(20) | DEFAULT `'user'` | 角色：`user` / `admin` |
| `max_watchlist_count` | INTEGER / Integer | DEFAULT 20 | 最大自选股数量 |
| `is_active` | INTEGER / Integer | DEFAULT 1 | 是否启用 |
| `created_at` | TEXT / String(20) | NOT NULL | — |
| `updated_at` | TEXT / String(20) | — | 更新时间 |
...
|------|------|------|------|
| `id` | INTEGER / Serial | PK, AutoIncrement | — |
| `user_id` | INTEGER / Integer | — | 用户ID（SQLite 有，PG 有） |
| `username` | TEXT / String(100) | NOT NULL | 用户名 |
| `action` | TEXT / String(200) | NOT NULL | 操作描述 |
| `details` | TEXT / String | — | 详情 |
| `ip_address` | TEXT / String(50) | — | IP 地址 |
| `created_at` | TEXT / String(20) | NOT NULL | — |
...
| `stock_code` | TEXT / String(10) | NOT NULL | — |
| `stock_name` | TEXT / String(100) | — | 股票名称 |
| `notes` | TEXT / String(500) | — | 备注（当前未使用） |
| `created_at` | TEXT / String(20) | NOT NULL | — |

- **唯一约束**：`(user_id, stock_code)`

#### 配置表

**`data_update_config`** — 数据更新配置（数据源、Token、定时任务参数）

| 列名 | 类型 | 约束 | 说明 |
|------|------|------|------|
| `id` | INTEGER / Serial | PK (SQLite), — | SQLite 有自增 ID；PG 以 `key` 为 PK |
| `key` | TEXT / String(100) | NOT NULL, UNIQUE / PK | 配置键 |
| `value` | TEXT / String(500) | NOT NULL | 配置值 |
| `updated_at` | TEXT / String(20) | NOT NULL | 更新时间 |

---

**SQLite 与 PostgreSQL 差异说明**

| 差异点 | SQLite | PostgreSQL |
|--------|--------|------------|
| 初始化方式 | `_init_database()` 手写 `CREATE TABLE` + 自动迁移（`ALTER TABLE ADD COLUMN`） | `_init_tables()` 使用 SQLAlchemy `Table()` + `metadata.create_all()` |
| 时间戳格式 | `%Y-%m-%d %H:%M:%S`（本地时间） | 同样使用 `%Y-%m-%d %H:%M:%S`（已统一） |
| `data_update_config` 主键 | `id` 自增 + `key` UNIQUE | `key` 为 PRIMARY KEY，无 `id` 列 |
| `users.updated_at` | 旧数据库通过迁移添加 | 原生包含 |
| `watchlist.stock_name/notes` | 旧数据库通过迁移添加 | 原生包含 |
| `audit_logs.user_id` | 原生包含 | 原生包含（已补齐） |
| `strategy_param_sets` 更新 | `ON CONFLICT DO UPDATE`（保留 ID） | `ON CONFLICT DO UPDATE`（保留 ID） |
- `src/strategy/`：
...
- **路径别名**：`@/` 映射到 `./src/`。
- **股票搜索**：`frontend/src/utils/stockSearch.ts` 提供拼音首字母简写搜索（如 `gzmt` → 「贵州茅台」），使用 `pinyin-pro` 库，带缓存机制。
...
Tushare `daily` 接口每次最多返回 **6000 条**数据，且支持 `ts_code='000001.SZ,600000.SH'` 逗号分隔批量查询。

- **全量更新**（无日期范围）：单只股票 23 年历史 ≈ 6000 条，恰好占满单次上限，因此每只股票仍需单独 1 次 `daily` + 1 次 `adj_factor` 请求。
- **增量更新**（有日期范围）：根据日期跨度智能计算 `batch_size`：
  - 1 个月（~22 条/只）→ 一次可拉 **~200 只**
  - 1 年（~250 条/只）→ 一次可拉 **~21 只**
  - 3 年（~750 条/只）→ 一次可拉 **~5 只**
- **复权因子同样批量**：`adj_factor` 接口也支持逗号分隔，与 `daily` 共享同一批分组，避免重复请求。

### AkShare（并发批量）

AkShare 的 `stock_zh_a_hist(symbol=...)` **不支持原生批量**，本质是东方财富网页爬虫。优化手段为 `ThreadPoolExecutor` 并发 I/O：

- **并发数**：默认 `AKSHARE_CONCURRENCY=4`，与 N100 的 4 线程匹配。
- **请求间隔**：每个 worker 内部保持 `AKSHARE_REQUEST_INTERVAL=0.8s`，IP 级别频率约 `4/0.8 = 5 次/秒`。
- **效果**：5000 只全量从串行 ~1.4h 降至并发 ~21min，提速 **4×**。

### 调用链路

```
StockDataFetcher.batch_fetch()
  ├─ TushareProvider → get_daily_data_batch() → 原生批量（ts_code 逗号分隔）
  └─ AkShareProvider → get_daily_data_batch() → ThreadPoolExecutor 并发
```

`batch_fetch` 通过 `hasattr(provider, 'get_daily_data_batch')` 自动检测 Provider 能力，不支持时 fallback 到逐只串行。
...
- **`adjust` 列与增量更新**：`get_latest_date`、`check_data_exists`、`get_all_latest_dates` 均已增加 `adjust` 参数。增量更新（如 `scripts/fetch_data.py` 的自动续拉）**必须传入 `adjust`**，否则可能因 hfq 数据存在而误判 qfq 已是最新，导致数据缺失。
- **`adjust` 默认值统一**：核心引擎（`StockDataFetcher` / `BaseDataProvider`）和后端 API 已全部统一为 `adjust="qfq"`。旧版本引擎默认 `hfq`、API 默认 `qfq`，混用会导致同一只股票存储两种复权数据，前端显示异常。
- **数据源切换与前端缓存**：`stockDataStore` 的缓存 key 已改为 `market:dataSource` 格式，切换数据源后会重新拉取股票列表，不会命中旧缓存。但 `ChartView.vue` 的 `rawKlineData` 是页面级内存缓存，切换数据源后需手动刷新或重新选择股票。
- **拼音首字母搜索性能**：`pinyin-pro` 首字母计算使用 `Map` 缓存，首次搜索 ~5000 只股票时约 50ms 初始化。后续搜索从缓存读取，无感知延迟。缓存为模块级全局变量，页面刷新后需重新计算。
